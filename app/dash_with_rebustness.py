...
ROBUSTNESS_DIR = PROJECT_ROOT / "metrics" / "robustness_analysis"
ROBUSTNESS_SUMMARY_CSV = ROBUSTNESS_DIR / "robustness_summary.csv"
ROBUSTNESS_DETAILS_CSV = ROBUSTNESS_DIR / "robustness_detailed_results.csv"
ROBUSTNESS_SUMMARY_IMAGE = ROBUSTNESS_DIR / "robustness_summary.png"

ROBUSTNESS_EXPERIMENT_LABELS = {
    "analyst_capacity": "Analyst capacity",
    "transaction_volume": "Transaction volume",
    "investigation_cost": "Investigation cost",
    "suppression_window": "Suppression window",
    "budget_multiplier": "Adaptive budget multiplier",
...
def load_robustness_summary() -> pd.DataFrame:
    """Load the saved overall robustness summary."""
    if not ROBUSTNESS_SUMMARY_CSV.exists():
        return pd.DataFrame()

    frame = pd.read_csv(ROBUSTNESS_SUMMARY_CSV)
    ...
    frame["display_parameter"] = frame["parameter_tested"].map(
        ROBUSTNESS_EXPERIMENT_LABELS
    ).fillna(
        frame["parameter_tested"]
        .astype(str)
        .str.replace("_", " ")
        .str.title()
    )
...
@st.cache_data(show_spinner=False)
def load_robustness_details() -> pd.DataFrame:
    """Load all point-level robustness results."""
    if not ROBUSTNESS_DETAILS_CSV.exists():
        return pd.DataFrame()

    frame = pd.read_csv(ROBUSTNESS_DETAILS_CSV)

    if "experiment" in frame.columns:
        frame["display_experiment"] = frame["experiment"].map(
            ROBUSTNESS_EXPERIMENT_LABELS
        ).fillna(
            frame["experiment"]
            .astype(str)
...
def robustness_totals(frame: pd.DataFrame) -> dict[str, int]:
    if frame.empty:
        return {
            "adaptive_wins": 0,
            "static_wins": 0,
    ...
    return {
        "adaptive_wins": int(frame["adaptive_wins"].sum()),
        "static_wins": int(frame["static_wins"].sum()),
        "ties": int(frame["ties"].sum()),
        "trade_offs": int(frame["trade_offs"].sum()),
        "tested_values": int(frame["tested_values"].sum()),
        "errors": int(frame["errors"].sum()),
    }


def format_robustness_summary(frame: pd.DataFrame) -> pd.DataFrame:
    display = frame[
        [
            "display_parameter",
            "adaptive_wins",
    ...
    return display.rename(
        columns={
            "display_parameter": "Parameter tested",
...
def format_robustness_details(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.empty:
        return frame
    ...
    existing = [column for column in wanted if column in frame.columns]
    display = frame[existing].copy()
...
robustness_summary_frame = load_robustness_summary()
robustness_details_frame = load_robustness_details()
robustness_overall = robustness_totals(robustness_summary_frame)
...
(
    ...
    [
        ...
        "4. Analyst Capacity",
        "5. Suppression Impact",
        "6. Adaptive Aggressiveness",
...
with robustness_tab:
    ...
    section_intro(
        "What does this section summarise?",
        (
            "It combines all completed one-factor-at-a-time sensitivity "
            "experiments and shows how often the Adaptive policy, the Static "
            "policy, a tie or a performance-cost trade-off occurred. Each "
            "experiment changes one parameter while the remaining assumptions "
            "stay fixed."
        ),
    )

    if robustness_summary_frame.empty:
        st.warning(
            "No saved robustness results were found. Run "
            "`py scripts\\run_robustness_analysis.py` from the project root. "
            "The dashboard will then load the generated CSV and figure "
            "automatically."
        )
    else:
        ...
        with r1:
            metric_card(
                "Adaptive wins",
                str(robustness_overall["adaptive_wins"]),
                (
                    f"across {robustness_overall['tested_values']} "
        ...
        with r2:
            metric_card(
                "Static wins",
                str(robustness_overall["static_wins"]),
                "settings where Static was operationally preferable",
                "red",
            )
        ...
        with r3:
            metric_card(
                "Ties",
                str(robustness_overall["ties"]),
                "settings with equivalent recall and cost",
                "blue",
            )
        ...
        with r4:
            metric_card(
                "Trade-offs",
                str(robustness_overall["trade_offs"]),
                (
                    "one policy improved recall while the other reduced "
                    "operational cost"
                ),
                "orange",
            )

        st.markdown("### Consolidated robustness table")

        summary_display = format_robustness_summary(
        ...
        adaptive_parameter_wins = int(
            (
                robustness_summary_frame["adaptive_wins"]
                > robustness_summary_frame["static_wins"]
            ).sum()
        )
        static_parameter_wins = int(
            (
                robustness_summary_frame["static_wins"]
                > robustness_summary_frame["adaptive_wins"]
            ).sum()
        )
        equal_parameter_outcomes = int(
            (
                robustness_summary_frame["static_wins"]
                == robustness_summary_frame["adaptive_wins"]
            ).sum()
        ...
        st.markdown(
            f"""
            <div class="takeaway">
                <strong>Overall robustness finding</strong><br>
                Adaptive produced more favourable outcomes in
                <strong>{adaptive_parameter_wins}</strong> of the
                <strong>{len(robustness_summary_frame)}</strong> tested parameter
                families. Static produced more favourable outcomes in
                <strong>{static_parameter_wins}</strong> parameter family, while
                <strong>{equal_parameter_outcomes}</strong> families had an equal
                number of Static and Adaptive wins. The results therefore support
                a generally stronger Adaptive policy, but not universal superiority.
                Static-threshold selection remains an important source of sensitivity.
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            """
            <div class="warning">
                <strong>Transaction-volume interpretation</strong><br>
                The transaction-volume experiment uses the same absolute analyst
                capacity for 1,000, 3,000, 10,000 and 50,000 transactions. It
                therefore evaluates increasing transaction workload under fixed
                analyst resources. It does not keep the analyst-capacity-to-volume
                ratio constant. A separate normalised-load experiment would be
                required to compare equal relative capacity across volumes.
            </div>
            """,
            unsafe_allow_html=True,
        )

        if ROBUSTNESS_SUMMARY_IMAGE.exists():
            st.markdown("### Summary figure")
            st.image(
                str(ROBUSTNESS_SUMMARY_IMAGE),
                caption=(
                    "Adaptive wins, Static wins, ties and trade-offs for each "
                    "tested parameter family."
                ),
                use_container_width=True,
            )

        st.markdown("### Experiment-by-experiment interpretation")

        interpretation_rows = [
            {
                "Parameter": "Analyst capacity",
                "Interpretation": (
                    "Adaptive usually performs better, particularly after "
                    "sufficient analyst capacity becomes available. Low-capacity "
                    "settings can produce ties or isolated Static advantages."
                ),
            },
            {
                "Parameter": "Transaction volume",
                "Interpretation": (
                    "Adaptive wins at 10,000 and 50,000 transactions, Static wins "
                    "at 3,000 and the policies tie at 1,000. This is a fixed-capacity "
                    "workload experiment rather than a proportional-capacity test."
                ),
            },
            {
                "Parameter": "Investigation cost",
                "Interpretation": (
                    "Adaptive remains preferable across all tested costs. Recall "
                    "does not change because investigation cost affects economic "
                    "evaluation rather than alert selection."
                ),
            },
            {
                "Parameter": "Suppression window",
                "Interpretation": (
                    "All tested windows preserve the same policy ordering. Because "
                    "recall is also unchanged, repeated-entity activity or the "
                    "practical effect of suppression may be limited in this sample."
                ),
            },
            {
                "Parameter": "Adaptive budget multiplier",
                "Interpretation": (
                    "Static performs better at multipliers 1.0–1.2, while Adaptive "
                    "performs better from 1.3 upward. This indicates an operational "
                    "transition point around 1.3."
                ),
            },
            {
                "Parameter": "Static threshold",
                "Interpretation": (
                    "Static wins at three of five tested thresholds. The baseline "
                    "threshold therefore materially affects the comparison, and "
                    "Adaptive superiority is not universal."
                ),
            },
        ]

        st.dataframe(
            pd.DataFrame(interpretation_rows),
            use_container_width=True,
            hide_index=True,
        )

        st.markdown("### Detailed robustness results")

        if robustness_details_frame.empty:
            st.info("The detailed robustness CSV is not available.")
        else:
            available_experiments = (
                robustness_details_frame["display_experiment"]
                .dropna()
                .drop_duplicates()
                .tolist()
            )
            ...
            selected_experiment = st.selectbox(
                ...
                ["All experiments", *available_experiments],
                key="robustness_experiment_filter",
            )

            filtered_details = robustness_details_frame.copy()

            if selected_experiment != "All experiments":
                filtered_details = filtered_details.loc[
                    filtered_details["display_experiment"].eq(
                        selected_experiment
                    )
                ]

            st.dataframe(
                format_robustness_details(filtered_details),
                use_container_width=True,
                hide_index=True,
            )

            details_csv = filtered_details.to_csv(
                index=False
            ).encode("utf-8")

            st.download_button(
                "Download displayed robustness results",
                data=details_csv,
                file_name="robustness_dashboard_results.csv",
                mime="text/csv",
                use_container_width=True,
            )
        ...
        st.markdown(
            ...
            <div class="small-note">
                “Adaptive win” and “Static win” use a joint recall-and-cost
                comparison. A result is a win only when one policy is no worse on
                both metrics and strictly better on at least one. These counts may
                therefore differ from counts based on recall alone.
            </div>
            """,
            unsafe_allow_html=True,
        )
